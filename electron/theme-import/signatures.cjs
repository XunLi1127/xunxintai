...
const TYPES = {
  ...
  apng: { mediaType: 'image/png', test: b => b.subarray(0, 8).equals(Buffer.from('89504e470d0a1a0a', 'hex')) && b.includes(Buffer.from('acTL')) },
...
function inspectMediaBuffer(fileName, buffer) {
  ...
  let dimensions;
  try { dimensions = imageSize(buffer); } catch (_) { throw new Error('Unable to recognize image'); }
