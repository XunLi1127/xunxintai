...
- RED：新增“握手后已发快照 `seq=1` 时拒绝首个入站 `seq=3`”测试，当前实现错误接受；新增“合法行后拼接 16385+ UTF-8 字节未终止尾部”测试，当前实现错误保留连接。两项均先按预期失败。
- GREEN：会话收发共用严格连续序号，首 heartbeat 为 `seq=2`；每消费完整 NDJSON 行后重新按 UTF-8 字节检查剩余尾部。聚焦测试更新为 15/15。
